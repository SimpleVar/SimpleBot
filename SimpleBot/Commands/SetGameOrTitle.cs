...
namespace SimpleBot.Commands
{
  static class SetGameOrTitle
  {
    ...
    public static async Task SetTitle(Bot bot, string tagUser, string title)
    {
      if (title.Length == 0)
        return;
      ...
      bot.TwSendMsg("Title changed: " + title, tagUser);
    }

    public static async Task SearchGame(Bot bot, string tagUser, string query)
    {
    ...
    public static async Task SetGame(Bot bot, string tagUser, string query)
    {
      var (gameId, gameName) = await _searchGame(bot, tagUser, query).ConfigureAwait(true);
      if (gameId == null) return;
      await bot._twApi.Helix.Channels.ModifyChannelInformationAsync(bot.CHANNEL_ID, new ModifyChannelInformationRequest
    ...
    static async Task<(string id, string name)> _searchGame(Bot bot, string tagUser, string query)
    {
