namespace SimpleBot
{
  // NOTE: THESE VALUES MUST BE SET IN STONE (for counting uses per user)
  enum BotCommandId : long
  {
    ListCommands = 50,
    SetObsWebsocketUrl = 80,
    AddIgnoredBot = 100,
    RemoveIgnoredBot = 110,
    SlowMode = 120,
    SlowModeOff = 130,
    SetTitle = 200,
    SetGame = 210,
    StartPoll = 250,
    EndPoll = 251,
    DelPoll = 252,
    AddCustomCommand = 300,
    DelCustomCommand = 301,
    EditCustomCommand = 302,
    ShowCustomCommand = 303,
    Trip = 400, // to edit shader with pleasure look for project named FileWatchAndDoStuff
    ToggleVideo = 500,
    DeleteMyLastMessage = 600,
    ShowBrb = 750,
    SearchGame = 1000,
    GetCmdCounter = 1005,
    GetRedeemCounter = 1007,
    FollowAge = 1010,
    WatchTime = 1011,
    SongRequest_Request = 1200,
    SongRequest_Volume = 1201,
    SongRequest_SetVolumeMax = 1202,
    SongRequest_Next = 1203,
    SongRequest_GetPrev = 1204,
    SongRequest_GetCurr = 1205,
    SongRequest_ShufflePlaylist = 1208,
    SongRequest_WrongSong = 1209,
    SongRequest_MySongs = 1210,
    SongRequest_ClearQueue = 1211,
    SongRequest_BumpUser = 1212,
    Reminders_Add = 1300,
    Reminders_Show = 1301,
    Queue_Curr = 1500,
    Queue_Next = 1501,
    Queue_All = 1502,
    Queue_Clear = 1503,
    Queue_Join = 1504,
    Queue_Leave = 1505,
    Queue_Close = 1506,
    Queue_Open = 1507,
    SneakyJapan = 2000,
    SneakyJapan_Stats = 2001,
    SneakyJapan_NewGamePlus = 2002,
    SneakyJapan_Leaderboard = 2003,
    SneakyJapan_MinusOneExp = 2004,
    SneakyJapan_SetReqChatActivity = 2005,
    SneakyJapan_AddExp = 2006,
    Celsius2Fahrenheit = 2900,
    Fahrenheit2Celsius = 2901,
    CoinFlip = 3010,
    DiceRoll = 3020,
    Quote_Get = 4000,
    Quote_Add = 4010,
    Quote_Del = 4020,
    LearnHiragana = 5010,
    GetChessRatings = 6010,
    SearchWordDefinition = 7000,
    FIRST_CUSTOM_COMMAND = 100000, // currently unused
  }
}