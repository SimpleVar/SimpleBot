...
namespace TwitchLib.EventSub.Websockets
{
  ...
  public class EventSubWebsocketClient
  {
    ...
    public readonly string WEBSOCKET_URL = "wss://eventsub.wss.twitch.tv/ws";
    ...
    public async Task<bool> ConnectAsync()
    {
      ...
      var success = await _websocketClient.ConnectAsync(new Uri(WEBSOCKET_URL)).ConfigureAwait(true);
      if (!success)
        return false;

      _cts = new CancellationTokenSource();
      _ = Task.Factory.StartNew(ConnectionCheckAsync, _cts.Token, TaskCreationOptions.LongRunning, TaskScheduler.Default);
      return true;
    }
    ...
    public async Task DisconnectAsync()
    {
      ...
      try
      {
        await _websocketClient.DisconnectAsync().ConfigureAwait(true);
      }
      catch (Exception ex)
      {
        ErrorOccurred?.Invoke(this, new ErrorOccuredArgs { Exception = ex });
      }
    }
    ...
    public async Task<bool> ReconnectAsync()
    {
      if (_reconnectRequested)
      {
        ...
        if (!await reconnectClient.ConnectAsync(new Uri(WEBSOCKET_URL)).ConfigureAwait(true))
          return false;
        ...
        for (var i = 0; i < 200; i++)
        {
          ...
          if (_reconnectComplete)
          {
            ...
            if (oldRunningClient.IsConnected)
              await oldRunningClient.DisconnectAsync().ConfigureAwait(true);
    ...
    private async Task ConnectionCheckAsync()
    {
      while (_cts != null && _websocketClient.IsConnected && !_cts.IsCancellationRequested)
      {
        ...
        await Task.Delay(TimeSpan.FromSeconds(1), _cts.Token).ConfigureAwait(true);
      }
